...
const AddExpenseCategoryScreen = () => {
    ...
    useEffect(() => {
        const syncTemporaryCategories = async () => {
            ...
            if (isConnected) {
                try {
                    let temporaryCategories = await AsyncStorage.getItem('temporaryCategories');
                    if (temporaryCategories) {
                        ...
                        await Promise.all(temporaryCategories.map(async (category) => {
                            await axios.post('http://10.0.2.2:3000/AddExpenseCategory', {
                        ...
                        await AsyncStorage.removeItem('ExpenseCategories');
                    }
                } catch (error) {
    ...
    const AddCategory = async () => {
        try {
            ...
            if (!isConnected) {
                ...
                const response = await axios.post('http://10.0.2.2:3000/AddExpenseCategory', {
                    name: categoryNameToSend,
                    icon: selectedIcon,
                    color: selectedColor,
                    userId: userId,
