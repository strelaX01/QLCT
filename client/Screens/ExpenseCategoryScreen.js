...
const fetchCategories = useCallback(async () => {
    try {
        ...
        if (isConnected) {
            ...
            await AsyncStorage.setItem('ExpenseOfflineCategories', JSON.stringify(response.data));
        } else {
            const storedCategories = await AsyncStorage.getItem('ExpenseOfflineCategories');
...
const handleDeleteCategory = async (categoryId) => {
    try {
        ...
        await axios.post('http://10.0.2.2:3000/DeleteExpenseCategory', { categoryId });
...
const handleEditCategory = () => {
    if (modalCategory) {
        navigation.navigate('EditExpenseCategory', { categoryId: modalCategory.id, categoryName: modalCategory.category_name, categoryIcon: modalCategory.icon, categoryColor: modalCategory.color , categoryIcon: modalCategory.icon});
        closeModal();
    }
};
