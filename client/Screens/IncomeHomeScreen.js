...
const IncomeHomeScreen = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            if (userId) {
                fetchIncomeData(userId, moment(chosenDate).format('YYYY-MM-DD'));
            }
        }, [userId, chosenDate])
    );
...
const fetchIncomeData = useCallback(async (userId, date) => {
    try {
        ...
        if (isConnected) {
            ...
            Alert.alert('Thông báo', 'Không có kết nối mạng. Không thể tải dữ liệu thu nhập.');
            setIncomeData({});
        }
    } catch (error) {
        console.log('Lỗi khi tải dữ liệu thu nhập', error);
        Alert.alert('Thông báo', 'Lỗi khi tải dữ liệu thu nhập');
    }
}, []);
    ...
    const saveIncome = async () => {
        ...
        try {
            ...
            if (isConnected) {
                const response = await axios.post('http://10.0.2.2:3000/saveincome', incomeData);
                if (response.status === 201) {
                    Alert.alert('Thông báo', 'Lưu thu nhập thành công.');
                    setAmount('');
                    setDescription('');
                    setSelectedCategory(null);
                    fetchIncomeData(userId, moment(chosenDate).format('YYYY-MM-DD'));
                } else {
                    Alert.alert('Thông báo', 'Lưu thu nhập thất bại.');
                }
            } else {
                const existingOfflineData = JSON.parse(await AsyncStorage.getItem('offlineIncomeData')) || [];
                existingOfflineData.push(incomeData);
                await AsyncStorage.setItem('offlineIncomeData', JSON.stringify(existingOfflineData));
            ...
            Alert.alert('Thông báo', 'Đã xảy ra lỗi khi lưu thu nhập.');
            console.error(error);
        }
    };
    ...
    const syncOfflineData = async () => {
        try {
            const offlineData = JSON.parse(await AsyncStorage.getItem('offlineIncomeData')) || [];
            if (offlineData.length === 0) return;
    
            const isConnected = await isNetworkConnected();
            if (isConnected) {
                await Promise.all(offlineData.map(async (data) => {
                    await axios.post('http://10.0.2.2:3000/saveincome', data);
                }));
    
                await AsyncStorage.removeItem('offlineIncomeData');
    ...
    const handleViewIncome = () => {
        navigation.navigate('ListIncome' , { userId: userId ,date: moment(chosenDate).format('YYYY-MM-DD')} );
    }
        ...
        <View style={styles.container}>
            ...
            <View style={styles.revenueview}>
                <View style={styles.Reveue}>
                    <Text style={styles.TextReveue}>Tiền thu</Text>
                </View>
            ...
            <View>
                <View style={styles.categoryHeader}>
                    ...
                    <TouchableOpacity style={styles.iconpen} onPress={() => navigation.navigate('CategoryTabView', { initialIndex: 0 })}>
                ...
                <View style={styles.AddView}>
                    <TouchableOpacity style={styles.btnadd} onPress={saveIncome}>
                        <Text style={styles.textAdd}>Thêm thu nhập</Text>
                    </TouchableOpacity>
                </View>
                
                <View>
                    <TouchableOpacity style={styles.viewIncomeButton} onPress={handleViewIncome}>
                        <Text style={styles.viewincomeText}>Xem danh sách thu nhập</Text>
                    </TouchableOpacity>
                </View>
            </View>
            <Modal
...
export default IncomeHomeScreen
