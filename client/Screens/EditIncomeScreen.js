...
const EditIncomeScreen = () => {
    ...
    const [selectedCategory, setSelectedCategory] = useState({ id, name,icon, color });
    ...
    const fetchCategories = useCallback(async () => {
        try {
            ...
            if (isConnected) {
                ...
                await AsyncStorage.setItem('incomeCategories', JSON.stringify(response.data));
            } else {
                const storedCategories = await AsyncStorage.getItem('incomeCategories');
    ...
    const UpdateIncome = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://10.0.2.2:3000/Updateincome', incomeData);
            ...
            if (error.response) {
                ...
                switch (status) {
                    case 400:
                        ...
                        break;
                    case 401:
                        Alert.alert('Lỗi lưu thu nhập', 'Số tiền không được để trống');
                        break;
                    case 402:
                        Alert.alert('Lỗi lưu thu nhập', 'Ghi chú không được để trống');
                        break;
                    case 404:
                        Alert.alert('Lỗi lưu thu nhập', 'Không tìm thấy bản ghi để cập nhật');
                        break;
                    default:
                        Alert.alert('Lỗi lưu thu nhập', 'Lưu thu nhập không thành công. Vui lòng thử lại sau.');
                        break;
                }
            }
        }
    ...
    return (
        <View style={styles.container}>
            <View style={{flexDirection:'row', alignItems:'centers'}}>
                <AntDesign onPress={() => navigation.goBack()} style={{marginTop:30}} name="arrowleft" size={24} color="black" />
                <Text style={{ flex: 1, textAlign: 'center', fontSize: 20, fontWeight: 'bold', color: 'black', marginTop: 30 }}>Chỉnh sửa khoản thu</Text>
            </View>
            ...
            <View>
                ...
                <View style={styles.flastlistview}>
                    {categories.length > 0 ? (
                        <FlatList
                            ...
                            style={styles.flatList} // Thêm style này vào đây
                        />
                ...
                <View style={styles.AddView}>
                    <TouchableOpacity style={styles.btnadd} onPress={UpdateIncome}>
                        <Text style={styles.buttonText}>Chỉnh sửa khoản thu</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
